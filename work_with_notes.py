    ...
    def add_new_note(self, notes_lst):
        ...
        for i in 'Название заметки','Описание заметки':
            ...
            while not correct_data:
                new_note[i]=input(f'Введите данные для поля "{i}": ').strip().replace(';','.') # если вдруг попала точка с запятой в поле, то будем ее менять на точку, иначе взникнет проблема при распаковке данных программой из csv файла, т.к. разделителем принят символ точка с запятой
                if set(new_note[i])&set(letter_symbols)==set():
